...
onBeforeMount(async () => {
    moves.value = await(await fetch("https://polishededitor-backend.vercel.app/moves")).json()
})
</script>
...
<template>
  <div v-if="moves" class="flex flex-col gap-3">
    <div v-for="(move, index) in mon['Moves']">
      <Select class="w-full" v-model="mon['Moves'][index]" :options="moves[PF].map((move) => move['Name'])" filter/>
      <div class="grid grid-cols-2 gap-1 mt-3">
        <div>Type: {{ (getMove(move))["Type"].at(0).toUpperCase() + (getMove(move))["Type"].slice(1) }}</div>
        <div>Base Power: {{ (getMove(move))["Base Power"] }}</div>
        <div>Category: {{ (getMove(move))["Damage Category"].at(0).toUpperCase() + (getMove(move))["Damage Category"].slice(1) }}</div>
