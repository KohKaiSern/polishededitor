...
for (const PF of ['polished', 'faithful'] as const) {
  keyItems[PF] = extractIDs(keyItems[PF], files[0][PF], NULL_KEY_ITEM, 'NUM_CANDIES', 'NUM_KEY_ITEMS');
  ...
  keyItems[PF] = extractPaths(keyItems[PF], files[3][PF], files[4][PF], 0, 'NUM_ITEMS', 'NUM_KEY_ITEMS');
