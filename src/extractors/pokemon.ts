...
import type { MonList, Mon, Ability, Move, GrowthRate, Species, FormMap, Form } from './types';
...
function extractPokemon(
    ...
    if (mon.id)
      if (mon.id in forms) {
        for (const form of forms[mon.id]) {
          const result = mons.contents.find((m) => m.index === form.index)! as Form;
          result.id = form.id;
  ...
  for (const mon of pokemon) {
    if (!mon.forms.find((form) => form.index === 1)) {
      const result = mons.contents.find((m) => m.index === mon.index)! as Form;
      result.name = 'Plain';
      result.dexNo = result.index;
      result.index = 1
      mon.forms.push(result);
    }
  }
...
for (const filename of readdirSync(
  ...
  const BASE = await splitRead('data/pokemon/base_stats/' + filename);
...
const files = await Promise.all(
  [
    ...
    'data/pokemon/variant_forms.asm'
...
for (const PF of ['polished', 'faithful'] as const) {
  mons[PF] = extractIDs(mons[PF], files[0][PF], NULL_MON);
  mons[PF].contents = extractNames(mons[PF].contents, files[1][PF], 0);
  ...
  mons[PF] = extractLvlMoves(mons[PF], files[3][PF], files[4][PF], moves[PF]);
  mons[PF] = extractEggMoves(mons[PF], files[5][PF], files[6][PF], moves[PF]);
  mons[PF] = extractEvoMoves(mons[PF], files[7][PF], moves[PF]);
  mons[PF] = extractSpritePaths(mons[PF], files[8][PF], files[9][PF]);
  mons[PF] = extractPalPaths(mons[PF], files[10][PF]);
  mons[PF] = extractAnimPaths(mons[PF], files[11][PF], files[12][PF]);
  forms[PF] = extractForms(mons[PF], forms[PF], files[0][PF], files[13][PF]);
