...
export function extractIDs<T extends Base>(data: T[], IDS: string[], empty: T, start: number, stop?: string): T[] {
  let index = start;
...
export function extractDescs<T extends Base & { description: string }>(data: T[], DESCS: string[], start: number, stop?: string): T[] {
  let index = start;
