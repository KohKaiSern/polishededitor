...
function reverseParseBoxes(file: Uint8Array, boxes: Box[], PF: 'polished' | 'faithful'): Uint8Array {
...
function reverseParseBoxThemes(file: Uint8Array, boxes: Box[], PF: 'polished' | 'faithful'): Uint8Array {
  for (let box = 0; box < 20; box++) {
    file[addresses.sBackupNewBox1 + 33 * box + 32] = boxThemes[PF].find(b => b.name === boxes[box].theme)!.index
  }
  return file
}
