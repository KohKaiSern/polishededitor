...
export function reverseParsePartyMon(file: Uint8Array, address: number, mon: PartyMon, PF: 'polished' | 'faithful'): Uint8Array {
...
export function reverseParseBoxMon(file: Uint8Array, address: number, mon: BoxMon, PF: 'polished' | 'faithful'): Uint8Array {
  file = species(file, address, mon.species, PF);
  file = form(file, address, mon.species, mon.form, PF);
  file = heldItem(file, address + 1, mon.heldItem, PF);
  file = moveset(file, address + 2, mon.moveset, PF);
...
const species = (file: Uint8Array, address: number, species: string, PF: 'polished' | 'faithful'): Uint8Array => {
...
const form = (file: Uint8Array, address: number, species: string, form: string, PF: 'polished' | 'faithful'): Uint8Array => {
...
const heldItem = (file: Uint8Array, address: number, heldItem: string, PF: 'polished' | 'faithful'): Uint8Array => {
  if (heldItem === 'None') {
    file[address] = 0
  } else {
...
const moveset = (file: Uint8Array, address: number, moveset: string[], PF: 'polished' | 'faithful'): Uint8Array => {
  for (let i = 0; i < 4; i++) {
    if (moveset[i] === 'None') {
      file[address + i] = 0;
    } else {
...
const shininess = (file: Uint8Array, address: number, shininess: 'Shiny' | 'Not Shiny'): Uint8Array =>
  (file[address] = (file[address] & 0x7F) | ((shininess === 'Shiny' ? 1 : 0) << 7), file)

const ability = (file: Uint8Array, address: number, species: string, form: string, ability: string, PF: 'polished' | 'faithful'): Uint8Array => {
  const mon = pokemon[PF].find(p => p.name === species)!
...
const gender = (file: Uint8Array, address: number, gender: string): Uint8Array =>
  (file[address] = (file[address] & 0x7F) | ((gender === 'Female' ? 1 : 0) << 7), file)

const isEgg = (file: Uint8Array, address: number, isEgg: boolean): Uint8Array =>
  (file[address] = (file[address] & 0xBF) | ((isEgg ? 1 : 0) << 6), file)
...
const boxPPUPs = (file: Uint8Array, address: number, PPUPs: number[]): Uint8Array =>
  (file[address] = PPUPs.reduce((a, b) => (a << 2) | b), file)
...
const caughtTime = (file: Uint8Array, address: number, caughtTime: string): Uint8Array =>
  (file[address] = (file[address] & 0x1F) | (['Evening', 'Morning', 'Day', 'Night'].findIndex(t => t === caughtTime) << 5), file)

const caughtBall = (file: Uint8Array, address: number, caughtBall: string, PF: 'polished' | 'faithful'): Uint8Array =>
  (file[address] = (file[address] & 0xE0) | items[PF].find(i => i.name === caughtBall)!.index, file)

const caughtLevel = (file: Uint8Array, address: number, caughtLevel: number): Uint8Array =>
  (file[address] = caughtLevel, file)

const caughtLocation = (file: Uint8Array, address: number, caughtLocation: string, PF: 'polished' | 'faithful'): Uint8Array =>
  (file[address] = locations[PF].find(l => l.name === caughtLocation)!.index, file)
...
const hyperTraining = (file: Uint8Array, address: number, hyperTraining: boolean[]): Uint8Array =>
  (file[address] = hyperTraining.map(s => Number(s)).reduce((a, b) => (a << 1) | b), file)
