...
function reverseParseParty(file: Uint8Array, party: PartyMon[], PF: 'polished' | 'faithful'): Uint8Array {
  ...
  for (let i = 0; i < party.length; i++) {
    ...
    file[addresses.wPartyMon1HyperTraining + i * 11] = party[i].hyperTraining.map(s => Number(s)).reduce((a, b) => (a << 1) | b)
